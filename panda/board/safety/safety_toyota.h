...
static int toyota_rx_hook(CANPacket_t *to_push) {
  ...
  if (valid && (GET_BUS(to_push) == 2U)) {
    ...
    if ((addr == 0x412) && !toyota_mads_lta_msg) {
      bool init_set_me_lock = false;
      int init_set_me = -1;
      bool set_me = (GET_BYTE(to_push, 0) & 0xC0) > 0; // LKAS_HUD
      if (controls_allowed == 0) init_set_me_lock = false;
      if ((alternative_experience & ALT_EXP_ENABLE_MADS) || (alternative_experience & ALT_EXP_MADS_DISABLE_DISENGAGE_LATERAL_ON_BRAKE)) {
        if (!init_set_me_lock) {
          if (!set_me) {
            init_set_me_lock = true;
            set_me_prev = set_me;
            init_set_me = 1;
          } else {
            init_set_me_lock = true;
        ...
        if (init_set_me_lock) {
          ...
          } else if (init_set_me == 2) {
            if (set_me_prev != set_me) {
              controls_allowed = 1;
            }
          }
        }
    ...
    if ((addr == 0x412) && toyota_mads_lta_msg) {
      bool init_set_me_lock = false;
      int init_set_me = -1;
      int set_me = (GET_BYTE(to_push, 3) & 0x40); // LDA_ON_MESSAGE
      if (controls_allowed == 0) init_set_me_lock = false;
      if ((alternative_experience & ALT_EXP_ENABLE_MADS) || (alternative_experience & ALT_EXP_MADS_DISABLE_DISENGAGE_LATERAL_ON_BRAKE)) {
        if (!init_set_me_lock) {
          if (set_me == 1) {
        ...
        if (init_set_me_lock) {
          if (init_set_me == 1) {
            if ((set_me == 1) && (set_me_prev != 1)) {
              controls_allowed = 1;
            }
          } else if (init_set_me == 2) {
            if ((set_me == 2) && (set_me_prev != 2)) {
              controls_allowed = 1;
            }
          }
        }
