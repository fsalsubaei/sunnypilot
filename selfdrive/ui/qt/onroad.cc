...
void OnroadWindow::mousePressEvent(QMouseEvent* e) {
  ...
  if (longitudinal_plan.getSpeedLimit() > 0.0 && speed_limit_touch_rect.contains(e->x(), e->y())) {
    ...
    if (gac_tr < 1) {
      ...
      else if (uiState()->scene.car_make == 2) gac_tr = 3;
      else if (uiState()->scene.car_make == 3) {
        if (gac_tr < 0) gac_tr = 3;
      }
    }
    if (gac_tr == 4) {
...
void NvgWindow::updateState(const UIState &s) {
  ...
  setProperty("distanceTraveled", cs.getDistanceTraveled());
...
void NvgWindow::drawHud(QPainter &p) {
  ...
  int rect_height = 204;
  if (!roadName.isEmpty()) {
    if (speedLimitStyle == 0) {
      rect_height = 432;
    }
    else if (speedLimitStyle == 1) rect_height = 392;
  }
  else if (has_us_speed_limit) rect_height = 402;
  ...
  if ((!roadName.isEmpty() && speedLimitStyle == 0) || has_us_speed_limit) {
    ...
    speed_limit_offset_rect.moveTop(sign_rect_outer.top() + 85 + 90);
  ...
  if ((!roadName.isEmpty() && speedLimitStyle == 1) || has_eu_speed_limit) {
    ...
    speed_limit_offset_rect.moveTop(center.y() + 30);
    p.setPen(QColor(0, 0, 0, 255));
    p.drawText(speed_limit_offset_rect, Qt::AlignCenter, slcSubText);
  }
...
void NvgWindow::drawGacButton(QPainter &p, int x, int y, int w, int h) {
  ...
  if ((prev_gap_adjust_cruise_tr != gapAdjustCruiseTr) || (!lock_init && (init_gap_adjust_cruise_tr != uiState()->scene.gap_adjust_cruise_tr))) {
    ...
    if ((gapAdjustCruiseTr == 4) || (uiState()->scene.gap_adjust_cruise_tr == 4) || ((uiState()->scene.car_make == 2) && (gapAdjustCruiseTr == 0) || (uiState()->scene.gap_adjust_cruise_tr == 0))) {
      gac_text = "Chill\nGap";
      gac_border = QColor("#044389");
    } else if ((gapAdjustCruiseTr == 3) || (uiState()->scene.gap_adjust_cruise_tr == 3)) {
      gac_text = "Normal\nGap";
      gac_border = QColor("#24a8bc");
    } else if ((gapAdjustCruiseTr == 2) || (uiState()->scene.gap_adjust_cruise_tr == 2)) {
      gac_text = "Aggro\nGap";
      gac_border = QColor("#fcff4b");
    } else if ((gapAdjustCruiseTr == 1) || (uiState()->scene.gap_adjust_cruise_tr == 1)) {
      gac_text = "Maniac\nGap";
