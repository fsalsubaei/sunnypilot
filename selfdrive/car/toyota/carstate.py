...
class CarState(CarStateBase):
  def __init__(self, CP):
    ...
    self.lkas_icon = None
    self.init_mads_enabled_lock = False
    self.init_mads_enabled = False
  ...
  def update(self, cp, cp_cam):
    ...
    if self.CP.carFingerprint in FEATURES["use_lta_msg"]:
      ...
      self.lkas_icon = cp_cam.vl["LKAS_HUD"]["LKAS_STATUS"] != 0
    else:
      self.mads_enabled = cp_cam.vl["LKAS_HUD"]["LKAS_STATUS"] != 0
    ...
    if ret.cruiseState.available:
      if self.gap_adjust_cruise:
        if self.gap_adjust_cruise_mode in (0, 2):
          ...
          self.gap_adjust_cruise_tr = cp.vl["PCM_CRUISE_SM"]["DISTANCE_LINES"]
          Params().put("GapAdjustCruiseTr", str(self.gap_adjust_cruise_tr))
      if self.enable_mads:
        if self.CP.carFingerprint in FEATURES["use_lta_msg"]:
          if not self.lkas_icon:
            self.madsEnabled = False
            self.init_mads_enabled_lock = False
          elif not self.init_mads_enabled_lock:
            if self.mads_enabled == 1:
              self.init_mads_enabled = 1
              self.init_mads_enabled_lock = True
            elif self.mads_enabled == 2:
              self.init_mads_enabled = 2
              self.init_mads_enabled_lock = True
          if self.init_mads_enabled_lock:
            if self.init_mads_enabled == 1:
              if self.prev_mads_enabled != 1 and self.mads_enabled == 1:
                self.madsEnabled = True
              elif self.prev_mads_enabled != 2 and self.mads_enabled == 2:
                self.madsEnabled = False
            elif self.init_mads_enabled == 2:
              if self.prev_mads_enabled != 2 and self.mads_enabled == 2:
                self.madsEnabled = True
              elif self.prev_mads_enabled != 1 and self.mads_enabled == 1:
                self.madsEnabled = False
        else:
          if not self.init_mads_enabled_lock:
          ...
          if self.init_mads_enabled_lock:
            if self.init_mads_enabled == 1:
              if not self.prev_mads_enabled and self.mads_enabled:
                self.madsEnabled = True
              elif self.prev_mads_enabled and not self.mads_enabled:
                self.madsEnabled = False
            elif self.init_mads_enabled == 2:
              if self.prev_mads_enabled and not self.mads_enabled:
                self.madsEnabled = True
              elif not self.prev_mads_enabled and self.mads_enabled:
                self.madsEnabled = False
